...
public:
    DataB(const string& filename, char delimiter = ',') {
        ...
        if (file.is_open()) {
            string line;
            getline(file, line); // Leer la primera l�nea (encabezados)
...
public:
    ...
    // Funci�n para calcular la capacidad total en bytes
    unsigned long long calculateTotalCapacity() {
        ...
        for (const auto& rowTypes : dataTypes) {
            for (const auto& fieldType : rowTypes) {
                // Supongamos algunos tama�os t�picos para los tipos de datos
    ...
    unsigned long long calculateFieldTypeSize(const string& fieldType) {
        ...
        if (fieldType == "int") {
            ...
            return fieldType.size(); // Tama�o de la cadena de caracteres
        }
    }

    // Funcion para calcular la capacidad utilizada en bytes
    unsigned long long calculateUsedCapacity() {
        return calculateTotalCapacity(); // Suponiendo que todos los datos est�n cargados en memoria
    }

    // Funcion para calcular el espacio libre en bytes
    unsigned long long calculateFreeSpace() {
        // Supongamos que el disco tiene un tama�o m�ximo de 1TB (1 terabyte)
    ...
    void saveSelectedData(const string& outputFilename) {
        ...
        // Encontrar los �ndices de las columnas "Nombres", "Edad" y "Sexo"
    ...
    // Verificar si se encontr� la columna
    if (columnIndex == -1) {
        cout << "La columna '" << columnName << "' no se encontr� en el archivo." << endl;
        return;
    }

    // Encontrar el �ndice de la columna de ID (asumiendo que el nombre de la columna puede ser "ID", "id" o "Id")
    int idIndex = -1;
    for (size_t i = 0; i < headers.size(); i++) {
        string header = headers[i];
        transform(header.begin(), header.end(), header.begin(), ::tolower); // Convertir a min�sculas
...
void filterColumnData(const string& inputFilename, const string& outputFilename, char comparisonOperator, int threshold) {
    ...
    if (inputFile.is_open() && outputFile.is_open()) {
        ...
        while (getline(inputFile, line)) {
            // Parsear la l�nea para obtener el valor a comparar
            // Suponemos que el ID est� en la primera columna y el valor a comparar en la segunda columna
            string id, valueStr;
            int value;
            ...
            // Realizar la comparaci�n seg�n el operador proporcionado por el usuario
            ...
            switch (comparisonOperator) {
                ...
                case '!':
                    ...
                    break;
                default:
                    cout << "Operador de comparaci�n no v�lido." << endl;
                    break;
            }

            // Si la fila cumple con la condici�n, copiarla al archivo de salida
            if (includeRow) {
